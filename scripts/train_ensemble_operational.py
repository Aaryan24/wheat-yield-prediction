    ...
    if cache_path.exists():
        ...
        if not np.array_equal(saved_seasons, np.array(seasons)) or not np.array_equal(saved_districts, np.array(district_ids)):
            ...
            for year in seasons:
                ...
                if np.isnan(yy).any():
                    # Should be handled, but raise here for consistency
...
def _run_ensemble_for_opdate(
    ...
    # Flatten
    train_x = embeddings[idx_train].reshape(-1, embeddings.shape[-1])
    val_x = embeddings[idx_val].reshape(-1, embeddings.shape[-1])
    test_x = embeddings[idx_test].reshape(-1, embeddings.shape[-1])
    ...
    # Predict
    # XGB
    train_xgb = xgb_model.predict(train_x)
    val_xgb = xgb_model.predict(val_x)
    test_xgb = xgb_model.predict(test_x)
    
    # RF
    train_rf = rf_model.predict(train_x)
    val_rf = rf_model.predict(val_x)
    test_rf = rf_model.predict(test_x)
    
    # Ensemble (Average)
    train_ens = (3*train_xgb + train_rf) / 4.0
    val_ens = (3*val_xgb + val_rf) / 4.0
    test_ens = (3*test_xgb + test_rf) / 4.0     
    # Analysis
    train_metrics = _metrics(train_y, train_ens)
    val_metrics = _metrics(val_y, val_ens)
    test_metrics = _metrics(test_y, test_ens)
    
    _log("Phase 4: Analyzing Ensemble performance...")
    bucket_counts = _analyze_errors(test_y, test_ens, "Test Set (Ensemble)")
    ...
    if pred_dir is not None:
        ...
        pred_frames = [
            _prediction_rows(
                ...
                y_pred=_reshape_pred(train_ens, len(train_years)),
                operational_label=operational_label,
            ),
            _prediction_rows(
                split_name="val",
                ...
                y_pred=_reshape_pred(val_ens, len(val_years)),
                operational_label=operational_label,
            ),
            _prediction_rows(
                split_name="test",
                ...
                y_pred=_reshape_pred(test_ens, len(test_years)),
                operational_label=operational_label,
            ),
        ]
        pred_df = pd.concat(pred_frames, ignore_index=True)
        pred_key = operational_label.replace("/", "-")
        pred_path = pred_dir / f"predictions_ensemble_opdate_{pred_key}.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train Ensemble (XGBoost + Random Forest) on Informer Embeddings."
    )
    ...
    parser.add_argument("--out-dir", type=str, default="experiments/ensemble_model")
