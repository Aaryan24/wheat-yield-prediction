...
def _train_one_operational_day(
    ...
    _log("Phase 2: Extracting embeddings from best Informer model...")
    with torch.no_grad():
        _, train_emb = model(train_w, train_wm, train_s, train_sm, adj_device)
        _, val_emb = model(val_w, val_wm, val_s, val_sm, adj_device)
        _, test_emb = model(test_w, test_wm, test_s, test_sm, adj_device)

    # Detach and Flatten: [S, N, D] -> [S*N, D]
    train_x_xgb = train_emb.detach().cpu().numpy().reshape(-1, train_emb.shape[-1])
    val_x_xgb = val_emb.detach().cpu().numpy().reshape(-1, val_emb.shape[-1])
    test_x_xgb = test_emb.detach().cpu().numpy().reshape(-1, test_emb.shape[-1])
    
    # Flatten targets: [S, N] -> [S*N]
    train_y_xgb = train_y.detach().cpu().numpy().ravel()
    val_y_xgb = val_y.detach().cpu().numpy().ravel()
    test_y_xgb = test_y.detach().cpu().numpy().ravel()

    # PHASE 3: Train XGBoost
    _log("Phase 3: Training XGBoost on embeddings...")
    ...
    train_pred_xgb = xgb_model.predict(train_x_xgb)
    val_pred_xgb = xgb_model.predict(val_x_xgb)
    test_pred_xgb = xgb_model.predict(test_x_xgb)

    # PHASE 4: Analysis
    _log("Phase 4: Analyzing XGBoost performance...")
    
    train_metrics = _metrics(train_y_xgb, train_pred_xgb)
    val_metrics = _metrics(val_y_xgb, val_pred_xgb)
    test_metrics = _metrics(test_y_xgb, test_pred_xgb)
    
    bucket_counts = _analyze_errors(test_y_xgb, test_pred_xgb, "Test Set (XGBoost)")
    ...
    if pred_out_dir is not None:
        ...
        pred_frames = [
            _prediction_rows(
                ...
                years=train_years,
                district_df=district_df,
                y_true=_reshape_pred(train_y_xgb, len(train_years)),
                y_pred=_reshape_pred(train_pred_xgb, len(train_years)),
                operational_label=operational_label,
            ),
            _prediction_rows(
                split_name="val",
                years=val_years,
                district_df=district_df,
                y_true=_reshape_pred(val_y_xgb, len(val_years)),
                y_pred=_reshape_pred(val_pred_xgb, len(val_years)),
                operational_label=operational_label,
            ),
            _prediction_rows(
                split_name="test",
                years=test_years,
                district_df=district_df,
                y_true=_reshape_pred(test_y_xgb, len(test_years)),
                y_pred=_reshape_pred(test_pred_xgb, len(test_years)),
                operational_label=operational_label,
            ),
        ]
        pred_df = pd.concat(pred_frames, ignore_index=True)
        pred_key = operational_label.replace("/", "-")
        pred_path = pred_out_dir / f"predictions_xgb_opdate_{pred_key}.csv"
        pred_df.to_csv(pred_path, index=False)

    # Save embeddings for all seasons with best model (optional/reused from original logic)
    with torch.no_grad():
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train Dual-Channel Informer (embeddings) + XGBoost yield model."
        )
    )
    ...
    parser.add_argument("--out-dir", type=str, default="experiments/informer_xgboost")
